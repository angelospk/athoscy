...
<a href="/ru/ekthesi/{data.url}" on:click={resetScroll}>
...
<p style="position:relative; top:90%;" class="my-1 text-2xl w-fit rounded-lg text-white bg-stone-500 bg-opacity-40">{data.ru}</p>
</div></a>
<!-- <p>{data.url}</p> -->
{/each}
</div>
...
<section class="card ">
    ...
    Доступные продукты нашей выставки загружены
